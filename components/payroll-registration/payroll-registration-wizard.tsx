...
type DocumentSlot = {
  key: string;
  label: string;
  category: "company-profile" | "legal" | "financial" | "payroll" | "commercial" | "supporting";
  required?: boolean;
};
...
const baseSteps = ["Company", "Contact", "Payroll", "Funding", "Documents", "Review"];
const services = [
  "Payroll Calculation",
  "PPh 21",
  "BPJS Kesehatan",
...
const benefits = [
  [Landmark, "Payroll Processing", "Perhitungan payroll, BPJS, pajak, dan komponen remunerasi."],
  [Building2, "Payroll Disbursement", "Workflow pembayaran gaji dengan approval dan reconciliation."],
  [CircleDollarSign, "Payroll Funding", "Opsi fasilitas pendanaan payroll berdasarkan hasil assessment."],
  [BarChart3, "Payroll Reporting", "Dashboard dan laporan payroll yang terstruktur dan transparan."],
] as const;
...
function fileError(slot: DocumentSlot, file: File) {
  ...
  if (!file.size || file.size > limit) {
    return `Ukuran maksimum ${limit / 1024 / 1024} MB untuk dokumen ini.`;
  }
  return "";
}
...
export function PayrollRegistrationWizard() {
  ...
  useEffect(() => {
    ...
    if (savedPending?.id && savedPending.registrationNumber && savedPending.clientToken) {
      setPending({ ...savedPending, uploaded: Array.isArray(savedPending.uploaded) ? savedPending.uploaded : [] });
    }
  ...
  const validateStep = () => {
    ...
    const missing = (required[current] ?? []).filter((key) => !String(draft[key] ?? "").trim());
    if (missing.length) {
    ...
    if (current === "Funding") {
      ...
      if (draft.workingCapital) {
        if (!String(draft.bankName ?? "").trim() || !String(draft.facilityType ?? "").trim() || Number(draft.facilityLimit) <= 0) {
    ...
    if (current === "Documents") {
      const missingDocuments = documentSlots.filter((slot) => slot.required && !hasDocument(slot.key));
      if (missingDocuments.length) {
        setError(`Dokumen wajib belum lengkap: ${missingDocuments.map((slot) => slot.label).join(", ")}.`);
        return false;
      }
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!application) {
        ...
        if (!response.ok) {
          const issue = data?.issues?.fieldErrors ? Object.values(data.issues.fieldErrors).flat().find(Boolean) : null;
        ...
        setPending(application);
        localStorage.setItem(PENDING_KEY, JSON.stringify(application));
      }

      const selected = documentSlots.filter((slot) => documents[slot.key] && !application.uploaded.includes(slot.key));
      let completed = 0;
      for (const slot of selected) {
        ...
        const uploaded = await fetch(`/api/payroll/applications/${application.id}/documents`, {
          method: "POST",
          body: upload,
        });
        const uploadData = await uploaded.json().catch(() => ({}));
        if (!uploaded.ok) {
          throw new Error(`${slot.label}: ${uploadData.error ?? "upload gagal"}. Klik Submit lagi untuk melanjutkan tanpa membuat registrasi baru.`);
        }
        application = { ...application, uploaded: [...application.uploaded, slot.key] };
        setPending(application);
        localStorage.setItem(PENDING_KEY, JSON.stringify(application));
        completed += 1;
      ...
      const mandatoryMissing = documentSlots.filter((slot) => slot.required && !application.uploaded.includes(slot.key));
      ...
      localStorage.setItem(TOKEN_KEY, application.clientToken);
      router.push(`/payroll/register/success?registration=${encodeURIComponent(application.registrationNumber)}`);
    } catch (reason) {
  ...
  const fieldSet = fields[baseSteps.indexOf(current)];
        ...
        <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-start xl:gap-12">
          ...
          <section className="min-w-0 rounded-3xl border bg-card p-5 shadow-sm sm:p-8">
            <header className="mb-7">
              ...
              {idaPrefilled && (
                <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-200">
                  ...
                  <p className="mt-1">Periksa semua field, koreksi bila perlu, dan lengkapi dokumen wajib. Submission tetap dilakukan manual oleh Anda.</p>
                  <button
                    type="button"
              ...
              {pending && (
                <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">
                  Registrasi <strong>{pending.registrationNumber}</strong> sudah dibuat. Jika upload sebelumnya gagal, Submit akan melanjutkan registrasi yang sama dan tidak membuat aplikasi duplikat.
                </div>
              )}
            </header>
            ...
            {fieldSet ? (
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                {fieldSet.map((field) => {
                  ...
                  return (
                    <div key={field.key} className={isAddress ? "sm:col-span-2" : ""}>
                      <Label htmlFor={field.key}>
                        {field.label}{field.optional ? <span className="font-normal text-muted-foreground"> · opsional</span> : null}
                      </Label>
                      {dropdowns[field.key] ? (
                        <select
                          id={field.key}
            ...
            {current === "Contact" && (
              <div className="mt-6 space-y-4">
                <Select
                  ...
                  value={String(draft.preferred ?? "WhatsApp")}
                  options={["WhatsApp", "Email", "Phone"]}
                  onChange={(value) => set("preferred", value)}
                />
                <CheckField
              ...
              <div className="mt-6 space-y-5">
                <div className="grid gap-5 sm:grid-cols-2">
                  <Select
                    ...
                    value={String(draft.frequency ?? "Monthly")}
                    options={["Monthly", "Bi-weekly", "Weekly", "Other"]}
                    onChange={(value) => set("frequency", value)}
                  />
                  <Select
                    label="Current Payroll System"
                    value={String(draft.currentSystem ?? "Excel")}
                    options={["Internal HR", "Excel", "HRIS", "Payroll Provider", "Outsourcing Provider", "Belum ada sistem"]}
                    onChange={(value) => set("currentSystem", value)}
                  />
                </div>
                <div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {services.map((service) => (
                      <CheckField
                        key={service}
                        label={service}
                        checked={Array.isArray(draft.services) && draft.services.includes(service)}
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(event) => set("challenge", event.target.value.slice(0, 3000))}
                    placeholder="Jelaskan kendala payroll saat ini, proses approval, cut-off, reporting, atau disbursement."
                  />
                </div>
              </div>
            )}
              ...
              <div className="mt-6 space-y-5">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="mt-2"
                    inputMode="numeric"
                    value={formatIdr(draft.fundingAmount)}
                    onChange={(event) => set("fundingAmount", currencyDigits(event.target.value))}
                    placeholder="Rp1.500.000.000"
                  />
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
                ...
                <CheckField
                  label="Perusahaan memiliki fasilitas working capital"
                  checked={Boolean(draft.workingCapital)}
                  onChange={(value) => set("workingCapital", value)}
                />
                {draft.workingCapital && (
                  <div className="grid gap-5 rounded-2xl border bg-muted/30 p-4 sm:grid-cols-2">
                    <div>
                      <Label htmlFor="bankName">Nama Bank</Label>
                      <Input id="bankName" className="mt-2" value={String(draft.bankName ?? "")} onChange={(event) => set("bankName", event.target.value)} />
                    </div>
                    <div>
                      <Label htmlFor="facilityType">Jenis Fasilitas</Label>
                      <Input id="facilityType" className="mt-2" value={String(draft.facilityType ?? "")} onChange={(event) => set("facilityType", event.target.value)} />
                    </div>
                    <div className="sm:col-span-2">
                      <Label htmlFor="facilityLimit">Limit Fasilitas</Label>
                      <Input id="facilityLimit" className="mt-2" inputMode="numeric" value={formatIdr(draft.facilityLimit)} onChange={(event) => set("facilityLimit", currencyDigits(event.target.value))} />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="mt-7 space-y-5">
                <div className="grid gap-4 sm:grid-cols-2">
                  <SummaryCard title="Company">
                    <p className="font-semibold">{String(draft.legalName ?? "—")}</p>
                ...
                <SummaryCard title="Requested Services">
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(draft.services) ? draft.services : []).map((service) => (
                      <span key={service} className="rounded-full border bg-background px-3 py-1 text-xs font-semibold">{service}</span>
                    ))}
                  </div>
                </SummaryCard>
                {funding && (
                  <SummaryCard title="Funding">
                    <p>Estimated Funding: <strong>{formatIdr(draft.fundingAmount) || "—"}</strong></p>
                    <p>Requested Term: <strong>{String(draft.paymentTerm ?? "—")}</strong></p>
                  </SummaryCard>
                )}
            ...
            <div className="mt-8 flex items-center justify-between gap-3 border-t pt-6">
              <Button variant="outline" disabled={step === 0 || busy || Boolean(pending)} onClick={() => { setError(""); setStep((value) => Math.max(0, value - 1)); }}>
                Previous
              </Button>
              {step < steps.length - 1 ? (
                <Button onClick={() => { if (validateStep()) setStep((value) => Math.min(steps.length - 1, value + 1)); }}>
                  Next
                </Button>
              ) : (
                <Button variant="accent" disabled={busy} onClick={submit}>
                  {busy ? "Submitting..." : pending ? "Continue Submission" : "Submit Registration"}
                </Button>
              )}
            </div>
            <p className="mt-4 text-right text-xs text-muted-foreground">
              Field form tersimpan otomatis. File upload harus dipilih ulang setelah browser direload.
            </p>
          </section>
        </div>
      </div>
...
function DocumentUploader({ documents, uploaded, onChange }: { documents: Record<string, File | undefined>; uploaded: string[]; onChange: (slot: DocumentSlot, file?: File) => void }) {
  return (
    <div className="mt-7">
      <div className="rounded-2xl border-2 border-dashed p-5 sm:p-6">
        <FileText className="h-9 w-9 text-[#0B3A6E]" aria-hidden />
        <h4 className="mt-3 font-semibold">Dokumen Initial Assessment</h4>
        <p className="mt-2 text-sm text-muted-foreground">
          PDF, XLS, XLSX, PNG, JPG/JPEG. Maksimum 10 MB per file; dokumen kategori financial maksimum 20 MB. NPWP, NIB, dan minimal satu financial document wajib.
        </p>
          ...
          {documentSlots.map((slot) => {
            ...
            return (
              <label
                ...
                onDrop={(event) => {
                  event.preventDefault();
                  onChange(slot, event.dataTransfer.files?.[0]);
                }}
                className="group rounded-xl border bg-background p-4 text-sm transition hover:border-[#0B3A6E]/40 hover:bg-muted/20"
              >
                ...
                <input
                  className="mt-3 block w-full text-xs file:mr-3 file:rounded-lg file:border-0 file:bg-muted file:px-3 file:py-2 file:font-semibold"
                  type="file"
