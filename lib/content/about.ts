/** Central About + homepage content — update here, not in page components. */
export const aboutContent = {
  whoWeAre: [
    "PT Mandiri Semesta Gemilang (MSG) is a Workforce Management Partner established in 2019.",
    "We integrate strategy, people, and technology to deliver end-to-end solutions that drive business transformation and sustainable growth.",
    "Our three core capabilities are Strategic Advisory, Workforce Solutions, and Technology, including ProQPay Lite as our integrated payroll platform.",
  ],
  moreThanManpower: {
    title: "People. Operations. Technology.",
    body: [
      "MSG combines strategic insight, reliable workforce operations, and practical technology in one accountable partnership.",
      "From business transformation and investment readiness to outsourcing, payroll services, managed operations, and digital integration, our solutions are designed around measurable outcomes.",
    ],
    quote: "One Partner. Integrated Solutions. Measurable Impact.",
  },
  pillars: [
    {
      id: "advisory",
      title: "Strategic Advisory",
      description:
        "Business recovery, growth strategy, investment readiness, M&A preparation, operational excellence, and business transformation advisory.",
    },
    {
      id: "workforce",
      title: "Workforce Solutions",
      description:
        "End-to-end workforce outsourcing, engineering talent, business support, and managed workforce operations.",
    },
    {
      id: "technology",
      title: "Technology",
      description:
        "Workforce management technology, data and analytics, and digital integration—led by ProQPay Lite.",
    },
  ],
  vision:
    "To be a trusted Workforce Management Partner that helps organizations grow through integrated strategy, people, operations, and technology.",
  mission:
    "To deliver flexible, integrated, technology-enabled solutions with disciplined operations, measurable outcomes, and long-term partnership.",
  coreServices: [
    {
      title: "Strategic Advisory",
      description:
        "Business recovery, growth, investment readiness, M&A advisory, and transformation.",
      href: "/services/strategic-advisory",
      benefit:
        "Featured service for companies preparing for the next stage of growth.",
      badge: "Featured Service",
    },
    {
      title: "Workforce Solutions",
      description:
        "Outsourcing, engineering talent, business support, and managed workforce under one umbrella.",
      href: "/services/workforce-solutions",
      benefit: "End-to-end people operations with structured continuity.",
    },
    {
      title: "Technology",
      description:
        "Workforce management technology, data and analytics, and digital integration.",
      href: "/technology",
      benefit:
        "Integrated technology with smarter operations and better outcomes.",
    },
  ],
  values: [
    {
      title: "Integrity",
      description:
        "Honest communication, transparent processes, and reliable commitments.",
    },
    {
      title: "Accountability",
      description: "Clear ownership of delivery, issues, and service outcomes.",
    },
    {
      title: "Service Excellence",
      description: "Disciplined execution and consistent service quality.",
    },
    {
      title: "Collaboration",
      description:
        "We operate as an extension of client teams and stakeholders.",
    },
    {
      title: "Agility",
      description:
        "Flexible service models that adapt to changing operational needs.",
    },
    {
      title: "Continuous Improvement",
      description:
        "Use operational data and feedback to refine delivery over time.",
    },
  ],
  howWeWork: [
    {
      step: 1,
      title: "Understand",
      description:
        "Understand business, workforce, and operational requirements.",
    },
    {
      step: 2,
      title: "Design",
      description: "Design the right advisory, service, or technology model.",
    },
    {
      step: 3,
      title: "Mobilize",
      description: "Mobilize talent, routines, and implementation support.",
    },
    {
      step: 4,
      title: "Operate",
      description: "Run workforce administration, operations, and delivery.",
    },
    {
      step: 5,
      title: "Measure",
      description: "Monitor performance, service quality, and business impact.",
    },
    {
      step: 6,
      title: "Improve",
      description:
        "Improve continuously using operational and business feedback.",
    },
  ],
  timeline: [
    {
      year: "2019",
      title: "Founded",
      description: "MSG established in Indonesia.",
    },
    {
      year: "2020–2023",
      title: "Service growth",
      description: "Expanded workforce, engineering, and operations support.",
    },
    {
      year: "2024–2026",
      title: "Advisory & technology",
      description:
        "Strengthened positioning as transformation partner; ProQPay advanced as flagship workforce technology.",
    },
  ],
  legalInfo: {
    companyName: "PT Mandiri Semesta Gemilang",
    founded: 2019,
    office: "Pondok Pinang Office Center No. 22, South Jakarta",
    email: "info@msg-os.com",
    phone: "+62 856-9766-6101",
    note: "Legal registration numbers and leadership details will be published after internal confirmation.",
  },
};

export const trustStrip = [
  { label: "Established", value: "2019" },
  { label: "Positioning", value: "Workforce Management Partner" },
  { label: "Capabilities", value: "Strategy · People · Technology" },
  { label: "Coverage", value: "Indonesian Businesses" },
  { label: "Technology", value: "ProQPay Enabled" },
];

export const whyMsg = [
  {
    title: "Improve",
    description:
      "Strengthen margins, cashflow, operating discipline, and workforce performance with structured assessment and action.",
  },
  {
    title: "Operate",
    description:
      "Run reliable day-to-day workforce operations—from deployment and administration to multi-site coordination.",
  },
  {
    title: "Transform",
    description:
      "Redesign organization, operations, workforce models, and digital roadmaps for sustainable business transformation.",
  },
  {
    title: "Grow",
    description:
      "Scale capacity and capability with flexible workforce models and growth-oriented advisory support.",
  },
  {
    title: "Scale",
    description:
      "Extend coverage across locations and functions with governance, technology visibility, and continuous improvement.",
  },
  {
    title: "Integrated delivery",
    description:
      "Strategic Advisory, Workforce Solutions, and Workforce Technology under one accountable partner—not fragmented vendors.",
  },
];

export const operationalCapabilities = [
  {
    title: "Strategic business assessment",
    description:
      "Health, operational, and financial diagnostics that inform recovery and growth decisions.",
    tag: "Advisory",
  },
  {
    title: "Large workforce mobilization",
    description:
      "Structured planning and deployment support for higher-volume operational roles.",
    tag: "Workforce",
  },
  {
    title: "Multi-location workforce coordination",
    description:
      "Operating routines designed for teams distributed across sites and regions.",
    tag: "Operations",
  },
  {
    title: "Payroll and administration management",
    description:
      "Coordination of workforce administration and payroll process support.",
    tag: "Operations",
  },
  {
    title: "Technical talent deployment",
    description:
      "Engineering and technical specialists mobilized for project timelines.",
    tag: "Engineering",
  },
  {
    title: "Technology-enabled visibility",
    description:
      "ProQPay and workforce technology for payroll control and operational insight.",
    tag: "Technology",
  },
];

/** Industry labels for homepage — industries we support, not claimed client logos. */
export const industriesWeSupport = [
  {
    title: "Technology and Telecommunications",
    href: "/industries/technology-telecommunications",
  },
  {
    title: "Construction and Engineering",
    href: "/industries/construction-engineering",
  },
  { title: "Financial Services", href: "/industries/financial-services" },
  { title: "Manufacturing", href: "/industries/manufacturing" },
  { title: "Energy and Utilities", href: "/industries/energy-utilities" },
  {
    title: "Logistics and Distribution",
    href: "/industries/logistics-distribution",
  },
  { title: "Retail and Consumer", href: "/industries/retail-consumer" },
  { title: "Professional Services", href: "/industries/professional-services" },
];

export const homepageClients = [
  "Enterprise Manufacturing",
  "Retail Networks",
  "Engineering Projects",
  "Logistics Operations",
  "Professional Services",
  "Financial Services",
];

export const portfolioHighlights = operationalCapabilities;

export const recruitmentFlow = [
  {
    step: 1,
    title: "Sourcing and attraction",
    description: "Reach candidates aligned to role requirements.",
  },
  {
    step: 2,
    title: "Screening and shortlisting",
    description: "Filter for capability and fit.",
  },
  {
    step: 3,
    title: "Interview and assessment",
    description: "Validate readiness with structured evaluation.",
  },
  {
    step: 4,
    title: "Hiring and onboarding",
    description: "Place talent with clear operational handover.",
  },
];

export const careerValues = [
  {
    title: "Meaningful work",
    description:
      "Contribute to workforce and transformation work that helps Indonesian organizations perform better.",
  },
  {
    title: "Professional growth",
    description:
      "Build experience across advisory support, people operations, service delivery, and technology-enabled workflows.",
  },
  {
    title: "Integrity and respect",
    description:
      "A culture that values accountability, collaboration, and fair treatment.",
  },
  {
    title: "Client impact",
    description:
      "See the connection between daily work and reliable outcomes for enterprise clients.",
  },
];

export const careerFraudWarning = [
  "MSG does not charge candidates any fees during the recruitment process.",
  "Verify all communication through official MSG channels before sharing personal data.",
  "Do not send sensitive documents to unverified parties claiming to represent MSG.",
];

export const openPositions: {
  id: string;
  position: string;
  location: string;
  employmentType: string;
  department: string;
  description: string;
  requirements: string[];
  applyHref: string;
  closingDate: string | null;
  status: "open" | "closed" | "draft";
}[] = [
  // No fabricated openings. Add real roles here when approved.
];

export const operationalExcellence = [
  {
    title: "Command center",
    description: "Coordination hub for multi-location operational oversight.",
  },
  {
    title: "Hybrid field supervision",
    description: "Field presence with centralized escalation paths.",
  },
  {
    title: "Standardized SOPs",
    description: "Shared operating standards across locations.",
  },
  {
    title: "Timely operational visibility",
    description:
      "Designed to provide timely visibility for managers and clients.",
  },
];

export const newsCategories = [
  "Business Transformation",
  "Operational Excellence",
  "Leadership",
  "Workforce",
  "Technology",
  "Industry Insights",
] as const;
